...
from app.models import Match, Result, Standing, Team, Tournament, db
...
def register_match_routes(app, db):
    ...
    def matches():
        ...
        query = Match.query.join(Tournament).filter(
    ...
    def match_result(id):
        ...
        if request.method == "POST":
            ...
            try:
                if scheduled_time_str:
                    match.scheduled_time = datetime.strptime(
        ...
        return render_template(
            "matches/matches_result.html", match=match, result=existing_result
        )
